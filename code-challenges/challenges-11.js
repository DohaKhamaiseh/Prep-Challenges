...
function gradesAvg(arr) {
  let avg=0; let arrObj = [];
 arrObj = arr.map((item,i)=>
   { avg = 0;
     for(let j=0; j<item.gradsList.length; j++)
     {avg += item.gradsList[j];}
     avg = avg/item.gradsList.length;
     item.avg=avg;
    return item;
   }); 
return arrObj;
...
function studentsResult(arr) {
  ...
  let arrResult = arr2.map((item,i)=>{
    if (item.avg<50)
    {item.result='Failed';}
    else {item.result='Passed';}
  return item;});
return arrResult; 
}
// -------------------------------------------------------------------------------------------------------
