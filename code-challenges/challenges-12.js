...
const cvsFiltration = (arr) => {
   ...
   for (let j=0; j<CVs.length; j++){
     if (CVs[j].firstName==null){
       let obj = {
         fullName : CVs[j].LastName,
         tech : CVs[j].tech } ; ar.push(obj);   }
     else if (CVs[j].LastName==null)
     {
        let obj = {
         fullName : CVs[j].firstName,
         tech : CVs[j].tech } ; ar.push(obj);  }
     else {
       let obj = {
         fullName : CVs[j].firstName + " " + CVs[j].LastName,
         tech : CVs[j].tech } ; ar.push(obj);  
      }
...
const skillComparison = (arr1, arr2) => {
 ...
 let result2 = arr2.filter((item)=>  ! arr1.includes(item)  )
