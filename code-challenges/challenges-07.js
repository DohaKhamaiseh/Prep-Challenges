...
const objLat = (obj) => {
    ...
    return "my name is " + firstname2 + " " + lastname2 + " I am " + obj.age + " YO, and I love " + obj.hobby + ".";
    ...
    for (let i = 0; i < arr.length; i++) {
        if (arr[i].yearsOfExperience > 1) {
            if (arr[i].lastName == null) {
                let obj = { "fullName": arr[i].firstName, "tech": arr[i].tech };
                arr2.push(obj);
            }
            else {
                let obj = { "fullName": arr[i].firstName + " " + arr[i].lastName, "tech": arr[i].tech };
                arr2.push(obj);
            }
        }
    }
...
const applicationsStatics = (arr) => {
    ...
    //let CP =0, CJS=0, CN=0, CJ=0, CR=0;
    for (let i = 0; i < arr.length; i++) {
        if (((arr[i].firstName == " " || arr[i].firstName == null) && (arr[i].lastName == " " || arr[i].lastName == null)) || arr[i].yearsOfExperience <= 1) { result.rejectedApplicants += 1; }
    ...
    result.totalApplicants = result.python_devs + result.javaScript_devs + result.dotNet_devs + result.java_devs + result.rejectedApplicants;
...
const classesAvg = (data) => {
    ...
    for (let g = 0; g < data.grades.length; g++) {
        for (let c = 0; c < data.grades[g].numberOfClasses; c++) {
            for (let s = 0; s < data.grades[g].classes[c].classScores.length; s++) { sum += data.grades[g].classes[c].classScores[s]; }
            data.grades[g].classes[c].avg = Math.floor(sum / data.grades[g].classes[c].classScores.length);
            sum = 0;
        }
